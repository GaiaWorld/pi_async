    ...
    fn drop(&mut self) {
        if let Some(waker) = {
            ...
            if status.0 <= UNLOCKED {
                ...
                panic!(
                    "Free shared lock failed, current: {:?}, reason: invalid current status",
                    status.0
                );
...
impl<T> RwLock<T> {
    ...
    pub async fn read(&self) -> RwLockReadGuard<T> {
        FutureShared { inner: self.inner.clone() }.await
    }

    /// 获取异步写锁
    pub async fn write(&self) -> RwLockWriteGuard<T> {
        FutureExclusive { inner: self.inner.clone() }.await
    }
}
...
impl<T> Future for FutureShared<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if status.0 >= UNLOCKED {
            ...
            return Poll::Ready(RwLockReadGuard { guarder: (&self).inner.clone() });
        }
...
impl<T> Future for FutureExclusive<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if status.0 == UNLOCKED {
            ...
            return Poll::Ready(RwLockWriteGuard { guarder: (&self).inner.clone() });
        }
