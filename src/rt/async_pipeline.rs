...
impl<T> Sink<T> for PipeSender<T> {
    ...
    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        if self.is_terminated.load(Ordering::Acquire) {
            ...
            return Poll::Ready(Err(Error::new(
                ...
                format!("Poll ready failed, reason: pipeline already terminated"),
            )));
        }
        ...
        if self.queue.is_full() {
            ...
            let mut spin_len = 1;
            loop {
                match self.status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed) {
                    Err(current) => {
                        ...
                        if spin_len >= 3 {
                            ...
                            return Poll::Ready(Err(Error::new(
                                ...
                                format!(
                                    "Poll ready failed, current: {}, reason: pipeline busy",
                                    current
                                ),
                            )));
                        }
    ...
    fn start_send(self: Pin<&mut Self>, item: T) -> Result<(), Self::Error> {
        if self.is_terminated.load(Ordering::Acquire) {
            ...
            return Err(Error::new(
                ErrorKind::BrokenPipe,
                format!("Start send failed, reason: pipeline already terminated"),
            ));
        }
        ...
        if let Err(_) = self.queue.push(item) {
            ...
            return Err(Error::new(
                ErrorKind::WouldBlock,
                format!("Start send failed, reason: pipeline already full"),
            ));
        }
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        if self.is_terminated.load(Ordering::Acquire) {
            ...
            return Poll::Ready(Err(Error::new(
                ErrorKind::BrokenPipe,
                format!("Poll flush failed, reason: pipelinne already terminated"),
            )));
        }
            ...
            match self.status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed) {
                Err(1) => {
                    ...
                    if spin_len >= 3 {
                        ...
                        return Poll::Ready(Err(Error::new(
                            ErrorKind::WouldBlock,
                            format!("Poll flush failed, current: 1, reason: pipeline busy"),
                        )));
                    }
            ...
            match self.status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed) {
                Err(1) => {
                    ...
                    if spin_len >= 3 {
                        ...
                        return Poll::Ready(Err(Error::new(
                            ErrorKind::WouldBlock,
                            format!("Poll close failed, current: 1, reason: pipeline busy"),
                        )));
                    }
                ...
                Err(_) => {
                    ...
                    let _ = self.is_terminated.compare_exchange(
                        false,
                        true,
                        Ordering::AcqRel,
                        Ordering::Relaxed,
...
pub fn channel<T>(capacity: usize) -> (PipeSender<T>, PipeReceiver<T>) {
    ...
    let receiver = PipeReceiver { is_terminated, queue, waker, status };
...
///
pub type BoxPipeline<'a, T, U = T, E = Error> =
    Pin<Box<dyn AsyncPipeLine<T, U, E> + Send + 'a, Global>>;

///
/// 异步管道，可以从异步管道中获取帧，也可以向异步管道中写入帧
///
pub trait AsyncPipeLine<StreamFrame, SinkFrame = StreamFrame, Err = Error>:
...
/// 扩展的异步管道
///
pub trait AsyncPipeLineExt<StreamFrame, SinkFrame = StreamFrame, Err = Error>:
...
impl<StreamFrame, SinkFrame, Err, T: ?Sized> AsyncPipeLineExt<StreamFrame, SinkFrame, Err> for T where
    T: AsyncPipeLine<StreamFrame, SinkFrame, Err>
{
}
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            if let Some(frame) = self.0.stream.pop() {
                ...
                //当前的读流无数据
                let mut spin_len = 1;
                match self.0.stream_status.compare_exchange(
                    0,
                    1,
                    Ordering::AcqRel,
                    Ordering::Relaxed,
                    ...
                    Err(3) => {
                        ...
                        unsafe {
                            if let Some(waker) = (*self.0.stream_waker.get()).take() {
                                //对端设置的唤醒器存在，则立即唤醒对端的写流
                                waker.wake();
                                self.0.stream_status.store(0, Ordering::Release);
                                //设置对端写流和当前读流的状态为已初始化
                            }
                        }
                        continue;
                    ...
                    Ok(_) => {
                        //对端写流或当前读流未异步等待唤醒，且已被当前调用锁住
                        unsafe {
                            *self.0.stream_waker.get() = Some(cx.waker().clone());
                            //设置读流的唤醒器
                        }
...
impl<T, U> FusedStream for AsyncDownStream<T, U> {
    ...
    #[inline]
    fn is_terminated(&self) -> bool {
        if self.0.is_terminated_stream.load(Ordering::Acquire) {
            //读流已终止，则继续判断当前读流是否为空
...
impl<T, U> Sink<U> for AsyncDownStream<T, U> {
    ...
    // 用户检查写流是否已满，如果当前写流已满，则异步等待对端读流消耗了写流的帧以后，再唤醒异步等待
    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        if self.0.is_terminated_sink.load(Ordering::Acquire) {
            //当前的写流已终止
            return Poll::Ready(Err(Error::new(
                ErrorKind::BrokenPipe,
                format!("Poll ready failed, reason: sink already terminated"),
            )));
        }
        ...
        if self.0.sink.is_full() {
            ...
            let mut spin_len = 1;
            loop {
                match self.0.sink_status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed)
                {
                    Err(current) => {
                        //对端读流或当前写流已经在异步等待唤醒，或者已被非当前调用锁住，则尝试指定次数的内部重试，重试失败则立即返回失败，但允许外部重试
                        if spin_len >= 3 {
                            ...
                            return Poll::Ready(Err(Error::new(
                                ErrorKind::WouldBlock,
                                format!(
                                    "Poll ready failed, current: {}, reason: sink or peer busy",
                                    current
                                ),
                            )));
                        }
                    ...
                    Ok(_) => {
                        //对端读流或当前写流未异步等待唤醒，且已被当前调用锁住
                        unsafe {
                            *self.0.sink_waker.get() = Some(cx.waker().clone());
                            //设置写流的唤醒器
                        }
    ...
    fn start_send(self: Pin<&mut Self>, item: U) -> Result<(), Self::Error> {
        if self.0.is_terminated_sink.load(Ordering::Acquire) {
            ...
            return Err(Error::new(
                ErrorKind::BrokenPipe,
                format!("Start send failed, reason: sink already terminated"),
            ));
        }
        ...
        if let Err(_) = self.0.sink.push(item) {
            ...
            return Err(Error::new(
                ErrorKind::WouldBlock,
                format!("Start send failed, reason: buffer already full"),
            ));
        }
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        if self.0.is_terminated_sink.load(Ordering::Acquire) {
            ...
            return Poll::Ready(Err(Error::new(
                ErrorKind::BrokenPipe,
                format!("Poll flush failed, reason: sink already terminated"),
            )));
        }
            ...
            match self.0.sink_status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed) {
                Err(1) => {
                    ...
                    if spin_len >= 3 {
                        ...
                        return Poll::Ready(Err(Error::new(
                            ErrorKind::WouldBlock,
                            format!("Poll flush failed, current: 1, reason: sink or peer busy"),
                        )));
                    }
            ...
            match self.0.sink_status.compare_exchange(0, 1, Ordering::AcqRel, Ordering::Relaxed) {
                Err(1) => {
                    ...
                    if spin_len >= 3 {
                        ...
                        return Poll::Ready(Err(Error::new(
                            ErrorKind::WouldBlock,
                            format!("Poll close failed, current: 1, reason: sink or peer busy"),
                        )));
                    }
                ...
                Err(_) => {
                    //对端读流或当前写流已经在异步等待唤醒，则立即终止对端读流和当前写流，然后唤醒对端读流或当前写流，并立即返回成功
                    //对端需要从对端读流获取所有剩余帧后，再终止对端读流
                    let _ = self.0.is_terminated_sink.compare_exchange(
                        false,
                        true,
                        Ordering::AcqRel,
                        Ordering::Relaxed,
